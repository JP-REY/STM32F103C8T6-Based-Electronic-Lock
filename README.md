...
- **Startup safe-state defense is not yet redundant.** CubeMX establishes PA8 low before `App_Init()`, but `App_InitLockActuator()` does not currently issue an explicit post-driver-initialization lock request.
