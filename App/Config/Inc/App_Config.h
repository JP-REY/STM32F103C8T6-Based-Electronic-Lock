...
/** @brief Required quiet interval after the most recent door-sensor edge, in milliseconds.                                       */
#define APP_DOOR_SENSOR_DEBOUNCE_TIME_MS         (500U)
...
typedef struct
{
    ...
    DoorSensor_Handle_t*   Door_Sensor;                  /*< Interrupt-oriented debounced door-sensor component instance.            */
    DoorSensor_Event_t*    Door_Sensor_Event;            /*< Most recent debounced door-sensor transition event.                     */
    DCS_SensorStatus_t*    Door_Sensor_Status;           /*< Output slot used for synchronous Door Control sensor-status queries.    */
    ...
    ExitButton_Event_t*    Exit_Button_Event;            /*< Event produced by the exit-button component for application processing. */
