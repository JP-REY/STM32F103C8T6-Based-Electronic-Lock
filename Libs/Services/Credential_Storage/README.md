...
* [13. Design Decisions](#13-design-decisions)
  ...
  * [13.6 Exclusive Linker-Reserved Page](#136-exclusive-linker-reserved-page)
...
CSS does not include STM32 HAL headers, use HAL page geometry or manipulate
...
* Page erase only when the current record location is not erased.
...
flowchart LR
    subgraph APP["Application Layer"]
        APP_CORE["App Core<br/>boot and action <br/>orchestration"]
    ...
    subgraph TARGET["STM32F103C8T6"]
        PAGE63["Internal Flash page 63<br/>reserved persistence"]
    end
...
* deciding when the dedicated page must be erased;
...
* STM32 HAL calls, page geometry or Flash register access;
...
CSS owns the first eight bytes of the final STM32F103C8T6 Flash page beginning
at:

```text
CSS_FLASH_BASE_ADDRESS = 0x0800FC00
```

The implementation obtains this address from linker symbol
`__credential_flash_start__`; it does not duplicate the numeric base in the C
source. The linker also exports `__credential_flash_end__` for map-file and
integration validation.

Page 63 spans `0x0800FC00` through `0x0800FFFF` and has a physical erase size
of 1 KiB. Although the credential record uses only eight bytes, the complete
page is reserved because erase operates at page granularity.

The active repository linker script separates executable and persistent Flash:

```text
FLASH             0x08000000 .. 0x0800FBFF   63 KiB
CREDENTIAL_FLASH  0x0800FC00 .. 0x0800FFFF   1 KiB
```

This prevents firmware sections from being linked into the page that CSS may
erase. No other module may place persistent data in page 63 without a new
...
`CSS_SaveCredential()` recognizes this erased state and skips the page erase:
...
Any non-erased current value requires page erase before a different record is
...
4. Erase the complete dedicated page.
...
The page shall never contain unrelated data because replacement erases all
1 KiB.
...
If power fails after an existing credential page is erased but before the new
...
* Erases the dedicated page only when the current location is not erased.
...
The STM32F1 HAL implements word and double-word programming as bounded
sequences of physical 16-bit writes. CSS intentionally keeps the logical
record transaction as two explicit 32-bit Platform operations so it can check
each result and place the marker and CRC together in the second word.

The record therefore cannot become valid after only the first Platform
operation. A failure inside either HAL word sequence is also detected by the
operation result and final readback validation.
...
### 13.6 Exclusive Linker-Reserved Page

The physical erase unit is much larger than the record. Reserving the complete
page prevents an update from deleting executable code or another module's
persistent data.

The linker enforces this ownership independently from the current firmware
size. Relying only on the image being smaller than `0x0800FC00` would be unsafe
...
| replacement save | unlock plus blocking 1 KiB page erase, two word programs, lock and readback |
...
Flash, including the credential record. The STM32F103C8T6 has no STM32F4-style
RDP Level 2 state; the Platform exposes its Level 0 and Level 1 states only.
...
7. Preserve exclusive ownership of STM32F103C8T6 page 63.
8. Keep the active linker script consistent with the CSS base address.
9. Do not place firmware sections or unrelated persistent records in page 63.
...
15. Expect a firmware-programmer mass erase to remove the credential unless the
    deployment procedure explicitly preserves page 63.
...
* replacing a credential performs one page erase and two word programs;
...
* executable `FLASH` ends before `0x0800FC00`;
* `CREDENTIAL_FLASH` begins at `0x0800FC00` and spans 1 KiB;
...
On STM32F103C8T6 hardware with recoverable firmware and known supply conditions:
...
* same-value save avoids page erase as observed through instrumentation;
* page 63 erase does not alter executable pages;
...
7. A complete 1 KiB page is reserved for an eight-byte V1 record.
8. The base address and Flash organization are STM32F103C8T6-specific.
...
Silently reinterpreting existing page contents is not acceptable.
